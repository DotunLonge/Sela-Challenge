...
export default ({project})=>{
    ...
    const alertOtherInfo = ()=>{
        alert("Start Date : " + new Date(project.startDate) + "\n End Date : " + new Date(project.endDate) )
    }
        ...
        <Wrapper className="xs-12 lg-6" statusColor = {project.projectStatus}>
            <div className="inner">
                ...
                <div className="projectStatus" > {project.projectStatus} </div>
                    <img src = {project.projectPicture} alt="projectPicture" className="projectPicture"/>
                    <div className="completionPercentage"> 

                    {percent > 0 && <Percentage id="blue" width={percent}>  
                ...
                <h3>{project.name}</h3>
                <div className="location xs-6 top"><img src={location} alt="location"></img><span>{project.location.name}</span> </div>
                <div className="budget xs-6 top"> <img src={budget} alt="budget"></img><span>Budget: {formatter(project.budget) }</span> </div>
                
                <div className="xs-12">
                    <p className='description'>{project.description} ...<Link to="#">Read more</Link> </p>
                </div>
                ...
                <h4>Contractors</h4>
                    {contractors}
                    <Link to="#" onClick={alertOtherInfo}> View more details </Link>
                </div>
